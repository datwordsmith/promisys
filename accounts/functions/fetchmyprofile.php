...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		$newdob = date("Y-m-d", strtotime($dob));
				...
				$check = mysqli_query($conn, "SELECT * FROM hod WHERE id = '$editid'");

				if (mysqli_num_rows($check) > 0) {
						
					$getPassword = mysqli_fetch_object(mysqli_query($conn, "SELECT * FROM hod WHERE id = '$editid'"));
					$existingPassword = $getPassword->password;
					
					if ($old == $existingPassword) {
						$edit = "UPDATE hod set password = '$new' WHERE id = '$editid'";
						$edited = mysqli_query($conn, $edit) or die(mysqli_error($conn));
...
$editid = $_SESSION["editprofile"]; //escape the string if you like
	...
	$staff = mysqli_fetch_object(mysqli_query($conn,  "SELECT * FROM hod WHERE id = '$editid'"));
