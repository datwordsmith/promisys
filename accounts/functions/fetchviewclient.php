		...
		if ($_SERVER["REQUEST_METHOD"] == "POST") {
				...
				$markup = (int)test_input($_POST["markup"]);
						...
						if ($balance >= $quantity) {	
							...
							$add = "INSERT INTO client_property (client_id, property_id, quantity, amount, discount, markup, staff_id, assigned_by) VALUES ('$clientid', '$propertyid', '$quantity', '$amount', '$discount', '$markup', '$staff', '$loginid')";
							$added = mysqli_query($conn, $add) or die(mysqli_error($conn));							
							...
							Echo "Quantity =".$propertyCount."<br/>";
							Echo "Sold =".$propertySold."<br/>";
