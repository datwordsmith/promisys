<?php
	...
	if(isset($_GET['month'])) {	
		...
		if ($monthid == "") {
			header("location: staffBirthdays");
		} else {
...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
        <i class="fa fa-calendar-plus-o"></i> Staff Birthdays - <?php echo $month;?>
        <small></small>
      </h1>
      <ol class="breadcrumb">
				  ...
				  <div class="box  box-warning">
					...
					<div class="box-body"  style="overflow-x:auto;">
							<table id="myTable" class="table table-striped table-bordered table-hover table-responsive" >
										<thead><tr><th>Date</th><th>Fullname</th> <th>Email</th> </tr> </thead>   
										
										<tbody>  
											<?php									
												
												$counter = 1;
												$all = mysqli_query($conn, "SELECT * from staff where MONTH(dob) = $monthid order by dob desc");
												while($staff = mysqli_fetch_object($all))
												{
														...
														echo '	
															<tr>
																	<td> '.$date.'</td> 														
																	<td> <i class="fa fa-'.strtolower($staff->sex).'" style="color: '.$sexcolor.'"></i> '.$fullname.'</td> 														
																	<td> '.$staff->email.'</td> 														
