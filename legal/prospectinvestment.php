<?php
	...
	//Declare Page
	$initialinvestment = "active";
	$pagename = "Initial Investments";
			 ...
			 <?php 
				...
				$amountdue = $getplan->amount;
		 		...
		 		if($payStatus == 2){
		 			echo '
				  	<div>
				  		<p class="text-success" style="font-size: 2em;"><i class="fa fa-check-square-o"></i> The Investment Advisor has been notified to pick up the Sales Agreement</p>
				  	</div>';
		 		} else {
					echo '
					<div class="checkbox" style="font-size: 2em;">
					    <label>
					      <input type="checkbox" class="confirm" > <span style="margin-left: 20px;">Invite Investment Advisor to pick up the Sales Agreement</span> 
					    </label>
					</div>

				  	<div>
				  		<button type="button" class="btn btn-success invitebtn" data-id="'.$prospectPropertyId.'" style="display: none;">Send Invitation</button>
				  	</div>';
				}
	...
	$(document).ready(function(){
		...
		$('.confirm').click(function(){
			if($('.confirm').prop("checked") == true){
				$('.invitebtn').fadeIn('slow');

			} else {
				$('.invitebtn').fadeOut('slow');
			}						
		});
		...
		$('.invitebtn').click(function(){
			...
			$.ajax ({
				...
				method:'POST',
				data:{invite: prospectPropertyId},
				dataType:'text',
				success:function(data) {
