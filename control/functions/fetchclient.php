...
if(isset($_POST['addclient'])) {
		...
		if (mysqli_num_rows($check) < 1) {
				...
				if ($added) {								
						...
						header("location: ../clients?failed");			
				}
		}
	
		else {
				
				header("location: ../clients?clientexists");	
		}	
}
														...
														<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
															  <div class="form-group">
																	  <div class="col-md-12 col-xs-12">
																			<input type="email" class="form-control" placeholder="Client Email" name = "email" pattern="^([\w-]+(?:\.[\w-]+)*)@((?:[\w-]+\.)*\w[\w-]{0,66})\.([a-z]{2,6}(?:\.[a-z]{2})?)$" required>														
																	  </div>
															...
															<div>
															  <button type="submit" name= "addclient" class="btn btn-default btn-flat btn-block ">Submit</button>
															</div>
...
$clientid = $_GET["viewclient"]; //escape the string if you like
