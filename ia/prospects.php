<?php
	...
	if(isset($_GET['addprospect'])) {
		header("location: functions/fetchprospect.php?addprospect");
	}
...
<script>
	...
	window.setTimeout(function() {
		$("#inactive-alert").fadeTo(500, 0).slideUp(500, function(){
			$(this).remove();
			var stateObj = {};
			window.history.pushState(stateObj, "", "prospects");
		});
	}, 4000);
...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
        <i class="fa fa-users"></i> My Prospects
        <small></small>
      </h1>
      <ol class="breadcrumb">
	...
	<!-- TABS -->
						...
						<?php
									...
									Prospect/Client already exists with the same email!
									</div>';
									}
	...
	<!--Project Menu -->
				...
				<div class="col-lg-12 col-xs-12">
					...
					<div style="float: right; margin-bottom: 10px;">
						<a href="prospects?addprospect" data-toggle="modal" data-target="#addBox" class="btn btn-primary btn-sm btn-info" ><i class="ace-icon fa fa-plus"></i> Add New Prospect</a>
					</div>
				  ...
				  <div class="box  box-warning">
					...
					<div class="box-body"  style="overflow-x:auto;">
							...
							<?php
									...
									$counter = 1;
									$all = mysqli_query($conn,  "SELECT * FROM prospects where staffid = '$loginid'");
									while($prospect = mysqli_fetch_object($all))
									{
											...
											echo '
												<tr>
														<td> '.$counter.'</td>
														<td> <i class="fa fa-'.strtolower($prospect->sex).'" style="color: '.$sexcolor.'"></i> <a href="viewprospect?details='.$prospect->id.'">'.$fullname.'</a></td>
														<td> '.$prospect->email.'</td>';

														if ($prospect->mobile == "") {
															echo '<td> '.$prospect->phone.'</td>';
														}

														else {
															echo '<td> '.$prospect->phone.', '.$prospect->mobile.'</td>';
														}

														echo '
														<td>
																<center>
																	<a href="viewprospect?details='.$prospect->id.'" class="btn btn-xs btn-info viewclient" ><i class="ace-icon fa fa-eye"></i> <span class="hidden-xs">View</span></a>
																</center>
														</td>
												</tr>';
												$counter++;
...
$('.viewprospect').click(function(){
    var viewprospect=$(this).attr('data-id');

    $.ajax({url:"functions/fetchprospect.php?viewprospect="+viewprospect,cache:false,success:function(result){
        $(".modal-content").html(result);
    }});
});
...
$('.editprospect').click(function(){
    var editprospect=$(this).attr('data-id');

    $.ajax({url:"functions/fetchprospect.php?editprospect="+editprospect,cache:false,success:function(result){
        $(".modal-content").html(result);
    }});
});
