...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		$projectid = $_SESSION['assignid'];
				...
				if (mysqli_num_rows($check) < 1) {
						...
						$add = "INSERT INTO project_property (project_id, property_type, quantity, amount) VALUES ('$projectid', '$propertytype', '$quantity', '$amount')";
						$added = mysqli_query($conn, $add) or die(mysqli_error($conn));
...
function test_input($data) {
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		$getProject = mysqli_fetch_object(mysqli_query($conn,  "select * from project_property where id = $propertyId"));
		$projectid = $getProject->project_id;

		//$edit = "UPDATE property set amount = '$amount' WHERE project_id = '$projectid' and type_id = '$type_id'";
		$edit = "UPDATE project_property set amount = '$amount' WHERE id = '$propertyId'";
		$edited = mysqli_query($conn, $edit) or die(mysqli_error($conn));

		if ($edited) {

			$add = "INSERT INTO change_type_price (user_id, property_id, previous, new) VALUES ('$loginid', '$propertyId', '$previous', '$amount')";
			$added = mysqli_query($conn, $add) or die(mysqli_error($conn));
...
if(isset($_POST['editunits'])) {
	...
	function test_input($data) {
		...
		if ($_SERVER["REQUEST_METHOD"] == "POST") {
				$newcount = test_input($_POST["newcount"]);
		}

			$propertyId = $_SESSION['editPropertyId'];
			$previous = $_SESSION['balance'];
			$loginid = $_SESSION['loginid'];
			...
			if ($newcount >= $propertySold) {
				//$edit = "UPDATE property set amount = '$amount' WHERE project_id = '$projectid' and type_id = '$type_id'";
				$edit = "UPDATE project_property set quantity = '$newcount' WHERE id = '$propertyId'";
				$edited = mysqli_query($conn, $edit) or die(mysqli_error($conn));
...
<div id="assign" class="modal-body">
									...
									<form  method="POST" action="<?php echo $_SERVER['PHP_SELF']; ?>" id="assign_form">
										<div class="form-group">
											<table id="myTable" class="table " >
													<thead><tr><th width=45%>Property Type</th> <th width=25%>Unit Cost (<?php echo $sign; ?>)</th> <th width=15%>Qty</th> <th></th></tr> </thead>
													<tr>
														<td>
														...
														<td width=25%><input type="number" class="form-control" min="1000000" placeholder="Unit Cost" name = "amount" id="amount" value="" required></td>
														<td width=15%><input type="number" class="form-control" min="1" placeholder="Qty" name = "quantity" id="quantity" value="" required></td>
							...
							<table id="myTable" class="table" >
										<thead><tr><th width=45%>Property Type</th> <th width=20%>Unit Cost (<?php echo $sign; ?>)</th> <th>Quantity</th> <th>Available</th> <th></th> </tr> </thead>
													...
													while($properties = mysqli_fetch_object($all))
													{
															...
															echo '
																<tr>
																		...
																		<td>'.$sign.number_format($pty->amount).'</td>
...
//echo $propertyId;
		...
		$_SESSION['previous'] = $property->amount;
?>
<div class="modal-header">
    <button type="button" class="close" data-dismiss="modal">&times;</button>
...
<span style="font-size: 18px; ">
	<?php echo $project->name.' - '.$property->property_type;?>
</span>
									<form  method="POST" action="<?php echo $_SERVER['PHP_SELF']; ?>" id="assign_form">
										<div class="form-group">
											<table id="myTable" class="table " >
													...
													<tr>
														...
														<td width=40%><input type="number" class="form-control" min="1000000" placeholder="New Price" name = "amount" id="amount" value="" required></td>
...
// EDIT UNITS MODAL
if(isset($_GET['editunits'])) {
	$propertyId = $_GET["editunits"]; //escape the string if you like
	...
	<div class="modal-header">
		<button type="button" class="close" data-dismiss="modal">&times;</button>
		<h4 class="modal-title">Edit Number of Units</h4>
	</div>
	...
	<span style="font-size: 18px; ">
		<?php echo $project->name.' - '.$property->property_type;?>
										...
										<form  method="POST" action="<?php echo $_SERVER['PHP_SELF']; ?>" id="assign_form">
											<div class="form-group">
												<table id="myTable" class="table " >
														<thead><tr><th width=45%><center>Current Total Units</center></th>  <th width=45%><center>New Total Units</center></th> <th></th></tr>
														</thead>
														<tr>
															<td>
																<input type="text" class="form-control" style="text-align: right;" value="<?php echo $propertyCount; ?>" disabled>
															</td>
															<td width=40%><input type="number" class="form-control" min="<?php echo $balance; ?>" placeholder="New Total Units" name = "newcount" id="amount" value="" required></td>

															<td><button type="submit" name= "editunits" class="btn btn-info btn-outline pull-right form-control"><i class="ace-icon fa fa-floppy-o"></i></button></td>
														</tr>
												</table>
											</div>
										</form>
