				...
				$unitcost = (int)test_input($_POST["amount"]);
						...
						if ($balance >= $quantity) {
							...
							if ($added) {
								    ...
								    $mail->addAddress('chukwuemeka.n@hall7projects.com', 'Chuk');     // Add a recipient
		   ...
		   $filename = stripslashes($_FILES['file']['name']);
			 $extension = getExtension($filename);				
			   ...
			   $doc = date("ymdHis").$_FILES['file']['name'];
				   $newname="../../offerletters"."/".$doc;
				   $copied = copy($_FILES['file']['tmp_name'], $newname);

				   if ($copied) {

					   $check = mysqli_query($conn, "SELECT * FROM prospect_property WHERE id = '$prospectProperty_id'");
	    
					   if ($check) {

					   		$checkfile = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM offerletters where prospectProperty_id = '$prospectProperty_id'"));
					   		if ($checkfile > 0) {
					   			//UPDATE
							   	$update = "UPDATE offerletters set file = '$doc' where prospectProperty_id = '$prospectProperty_id'";
								$updated = mysqli_query($conn, $update) or die(mysqli_error($conn));
													   
							   if ($updated) {

						   			//PROSPECT PROPERTY
								   	$updateProperty = "UPDATE prospect_property set rfo = 1 where id = '$prospectProperty_id'";
									$PropertyUpdated = mysqli_query($conn, $updateProperty) or die(mysqli_error($conn));							   	

									$getRecipient = mysqli_fetch_object($check);
									$recipient_id = $getRecipient->staff_id;

									$getStaff = mysqli_fetch_object(mysqli_query($conn,  "SELECT * FROM staff where id = '$recipient_id'"));
									$recipient = $getStaff->email;
								    ...
								    $mail->Body    = 'Hello,<p>You have a new Offer Letter awaiting download.</p><p>Kindly Visit <a href="http://promisys.hall7projects.com/">PROMISYS</a> for details.</p>';
								   ...
								   header("location: ../viewprospect?details=".$prospectid);	
								   echo $prospectProperty_id." - updated";
							   }
							   else {
								   $Failed = "Failed";
					   			...
					   			//ADD NEW
							   	$add = "INSERT INTO offerletters (file, prospectProperty_id) VALUES ('$doc', '$prospectProperty_id')";
								$added = mysqli_query($conn, $add) or die(mysqli_error($conn));
...
//INVITE FOR OFFER LETTER
if(isset($_POST['invite'])) {
	$prospectPropertyId = $_POST["invite"];

	$edit = "UPDATE prospect_property set rfo = '2' WHERE id = '$prospectPropertyId'";
	...
	if ($edited) {
	    ...
	    $mail->isHTML(true);  // Set email format to HTML
	    $mail->Subject = 'Request for Offer Letter';
	    $mail->Body    = 'Hello '.$staffname.',<p>You have a new Offer Letter awaiting pickup</p>.<p>Kindly Visit the Admin office.</p>';
...
if(isset($_GET['addofferletter'])) {
	$prospectProperty_id = $_GET['addofferletter'];
	...
	<div class="modal-body">
		<form action="functions/fetchviewprospect.php" method="POST" enctype="multipart/form-data">
			<div class="form-group col-md-12 col-xs-12">
				<input type="text" class="form-control" placeholder="File Title" name = "filetitle" value="Offer Letter" disabled>
			</div>
			...
			<div class="form-group col-md-12 col-xs-12">
				<span>&nbsp;</span>
				<button type="submit" name= "addfile" id="doc_upload" class="btn btn-default btn-block btn-flat"><i class="ace-icon fa fa-floppy-o"></i> Upload Document</button>
			</div>
		</form>		
	</div>
