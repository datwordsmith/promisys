					...
					<form  method="POST" action="<?php echo $_SERVER['PHP_SELF']; ?>" id="assign_form">
						<div class="col-md-4 col-xs-12">
								<select name="project" id="project" class="form-control" onChange="change_project()">
									...
									<?php
									$pjt = mysqli_query($conn,  "SELECT * FROM project where id in (select project_id from property) order by name");
...
?>
				...
				<div class="modal-body">
					...
					<p class="statusMsg"></p>
					<input type="hidden" value="<?php echo $clientPropertyId; ?>" id = "clientPropertyId" hidden>
					...
					<form method="post" id="planForm" action="">
						...
						<div class="col-md-2 col-xs-12">									
							<center>
								<!--<input type="button" id="submitPlan" class="btn btn-default" value="Submit" onclick="aa();">-->
								<button type="button" id="submitPlan" class="btn btn-info col-xs-12 submitPlan"><i class="ace-icon fa fa-floppy-o"></i></button>
							</center>
						</div>
						<div style="clear: both;"></div>
					</form>
					...
					<div id="exbox"> 
						<table id="myTable" class="table" >
							...
							<thead><tr><th>Investment</th> <th width=25%>Date</th> <th><span style="float: right;">Amount (<?php echo $sign; ?>)</span></th> <th></th> </tr> </thead>   
									
									<tbody class="exbody"> 
										<?php
											$counter = 1;
											
											$all = mysqli_query($conn,  "SELECT * FROM plan where clientproperty_id = '$clientPropertyId' order by id desc");
													...
													$date = strtotime($date);														
														...
														echo '
																<tr>
																	...
																	<td><span style="float: right;">'.$sign.$row->amount.'</span></td>

																	<td>
																		<a href="#" data-id="'.$row->id.'" class="btn btn-xs btn-danger deleteplan" ><i class="ace-icon fa fa-times"></i></a>
																	</td> 															
...
if(isset($_GET['addpayment'])) {	
		...
		$_SESSION["clientpropertyid"] = $clientPropertyId; 
			...
			$property = mysqli_fetch_object(mysqli_query($conn,  "SELECT * FROM property_type WHERE id = $propertyid"));
...
<div class="modal-body">
			...
			<table id="myTable" class="table " >
					...
					<tr><th width=30%>Property Type</th> <td><?php echo $property->propertytype; ?></td> </tr>
					<tr><th width=30%>Number of Units</th> <td><?php echo $detail->quantity; ?></td> </tr>
					<tr><th width=30%>Total Cost</th> <td><?php echo $sign.$detail->amount; ?></td> </tr>
					<tr><th width=30%>Amount Paid</th> <td><?php echo $sign.$amountpaid; ?></td> </tr>
					<tr><th width=30%>Balance</th> <td><?php echo $sign.$balance; ?></td> </tr>
			</table>
...
<div class="modal-body">
			...
			<table id="myTable" class="table " >
					...
					<tr><th width=30%>Amount Paid</th> <td><?php echo $sign.$amountpaid; ?></td> </tr>
					<tr><th width=30%>Balance</th> <td><?php echo $sign.$balance; ?></td> </tr>
					<tr><th width=30%>File ID</th> <td><?php echo $detail->fileid; ?></td> </tr>
...
if (isNaN(amount) || amount < 1 || amount.trim == '') {
 ...
 $("#flash").show();
 $("#flash").fadeIn(600).html('<span class="load"><i class="fa fa-spinner" aria-hidden="true"></i></span>');


 $("#exbox").fadeIn(400).html()
