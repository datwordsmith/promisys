						  ...
						  <div class="box  box-info">
							...
							<table id="myTable" class="table table-bordered table-striped"  style="padding-bottom: 50px">
									...
									<tbody>
										...
										//$counter = 1;
										$all = mysqli_query($conn,  "SELECT * FROM prospect_property where prospect_id = '$prospectid' order by rfo, date asc");
										while($prospectproperty = mysqli_fetch_object($all))
										{
											...
											$getOffer = mysqli_fetch_object(mysqli_query($conn, "select * from offerletters where prospectProperty_id = '$prospectPropertyId'"));
											if ($getOffer) {
											...
											if ($payStatus == 2) {
												$stat = '<button class="btn btn-xs btn-info" disabled>Payment Confirmed</button>';
											} else {
												$stat = '';
											}										
											...
											echo '
												<tr>
														...
														<td>
															<a href="#addBox" data-toggle="modal" data-target="#addBox" data-id="<?php echo $prospectPropertyId; ?>" class="btn btn-xs btn-warning uploaddoc"><i class="ace-icon fa fa-upload"></i> Upload File</a>
